...
open System
let languages = languageInfo.Split([|"\n"|], StringSplitOptions.RemoveEmptyEntries) |> Array.filter (fun x -> Char.IsWhiteSpace(x.[0]) = false)
let languagesToFile = String.Join(Environment.NewLine, languages)
let haha = IO.File.WriteAllText("c:\\trash\\github_languages.txt", languagesToFile)